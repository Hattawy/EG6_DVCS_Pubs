...
thanks to the theoretical and experimental developpements with three dimensional 
structure functions \cite{Anselmino:2015uka}. These studies can be generalized to the
nucleus and offer a unique opportunity to revisit the quark structure of the nucleus
with an original perspective. This new approach is particularly needed as the quark
structure of the nucleus remains today the subject of numerous
controvercies. Indeed, while much progress has been made in measuring the nuclear
parton distribution functions (PDFs), the observed shape of the PDF ratio, known as
EMC effect, remains fundamentaly unexplained \cite{Norton:2003cb,Malace:2014uea,Hen:2016kwk}.

In nuclei, the generalized parton distributions (GPDs) can be probed conveniently through
spin assymetries in the deeply virtual Compton scattering (DVCS) 
process~\cite{Diehl:2003ny,Belitsky:2005qn,Guidal:2013rya}. Indeed, the exclusive
production of a photon limits the possibilities of final state interactions in the nuclear 
medium. Moreover, with a nuclear targets of spin-0 the extraction of the GPD from the DVCS data 
is significantly simplified with the reduction to a single GPD intervening in the process. The 
measurement of the nuclear DVCS is however challenging experimentally, first attempts from 
the HERMES collaboration \cite{Airapetian:2009cga} lead to imprecise results. We present here
more recent and detailed measurements from the CLAS collaboration already partially presented
in previous letters \cite{Hattawy:2017woc,Hattawy:2018liu}. We extend in this article 
the description of the nuclear DVCS CLAS experiment, detail the methods used for the
data analysis and produce the full experimental results for each channel measured.
...
process in which % TODO add BH figure
...
\includegraphics[width=9cm]{fig1/finalplane.png}
	...
	in the DVCS process.} %TODO upgrade to a figure with phi_S
\label{fig:PhiAngle}
\end{figure}
...
helium nuclei. In principle, this could lead to significant target mass corrections...
REF? \cite{Braun:2012bg}
...
already reviewed many times in the litterature~\cite{Diehl:2003ny,Belitsky:2005qn,Guidal:2013rya}. 
...
a large contribution from such background reaction. 
...
BSA signal. Thier results, presented in Fig. \ref{fig:HERMES1} and \ref{fig:HERMES2}, suffer
from large error bars, which makes them consistent with the free proton data and prevents us to
reach strong conclusions about any nuclear effect. Yet, in the coherent DVCS case a rather
...
issue with this method is that the detector resolution may not be good enough to separate the 
...
helium recoil need to be measured to ensure the exclusivity in the coherent channel. In this section, we 
will rapidly describe the particular detector setup used in our experiment. 

\subsection{The CLAS detector}

The CLAS spectrometer \cite{Mecking:2003zu} is installed in the hall-B of the JLab main 
accelerator. It has for main objective to study the multi-particles final states
...
CLAS is composed of six identical radial 
...
particularly sensitive to the Moller electrons scattered from the target at small angles.
...
possible to have particles from different events getting combined and pass all the cuts and get 
...
We extract the BSA at 90° in Fig. \ref{fig:CohALU90}, the model 
