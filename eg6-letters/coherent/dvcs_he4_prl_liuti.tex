...
transverse plane for partons carrying a 
%certain 
\textcolor{magenta}{given fraction of the proton}
...
structure and allowed \textcolor{magenta}{for an} extraction of the tomography of the nucleon (for details 
on the formalism, see 
\textcolor{magenta}{\cite{Goeke:2001tz,Diehl:2003ny,Ji:2004gf,Belitsky:2005qn,Boffi:2007yc,Guidal:2013rya,Kumericki:2016ehc}}).
...
\textcolor{magenta}{A} study of the 3D structure of nuclei appears to be especially important
...
illustrates the hand bag diagram for coherent DVCS on $^4$He.  Similarly to the 
...
accessed experimentally. In the experiment we measure the Compton Form
Factors (CFF) \cite{Guidal:2013rya} \textcolor{magenta}{which are} complex amplitudes defined as:
...
The DVCS and %well-known 
Bethe-Heitler (BH) processes, where the real 
...
%can be decomposed into a finite sum of Fourier harmonics is written in terms of 
\textcolor{magenta}{depend on} the azimuthal angle
...
%depend 
\textcolor{magenta}{were written in Ref.\cite{Belitsky:2008bz})} as a function of  
Fourier harmonics in the azimuthal angle $\phi$, the nuclear form factor 
$F_A(t)$, and \textcolor{magenta}{the kinematic invariants}, $Q^2$, $x_B$, \textcolor{magenta}{$x_{A} = \displaystyle\frac{x_{B}M_{N}}{M_{A}}$, 
$y=\displaystyle\frac{Q^2}{2x_{A} M_A E_{beam}}$,
and $\epsilon = \displaystyle\frac{2x_{A}M_{A}}{\sqrt{Q^{2}}}$, where $M_{A}$($M_{N}$) is the $^4$He 
(nucleon) mass.} 
...
%in the leptonic tensor.  
The explicit expressions of these terms can be found 
...
S.~Liuti and K.~Taneja \cite{simonetta_2}. 
%The model relies on the impulse 
%approximation and uses advanced spectral functions of nuclei. 
\textcolor{magenta}{The model takes into account 
the effect on the quark distribution of the high virtuality/off-shellness of the nucleon with 
four-momentum squared not equal to the square of their mass.}
The calculations 
are at slightly different kinematics than our data but still provide some 
guidance. The experimental results appear to have \textcolor{magenta}{a larger asymmetry  than the 
calculation in the $x_B=0.307$ bin.  This difference arises from the theoretical 
uncertainty in the determination of the crossing
point between the antishadowing region, where the parton nuclear distribution is larger than the nucleon one, and 
the EMC effect region, where the nuclear effect reverses its sign.}
%These differences may arise from nuclear effects, such as 
...
{\it convolution} and {\it off-shell}. \textcolor{magenta}{The off-shell model is the same as in Fig.\ref{fig:alu90}, using an improved 
GPD model for the free nucleon \cite{GonzalezHernandez:2012jv}}. In the convolution model 
...
\cite{DD_model}. 
%The off-shell model \cite{GonzalezHernandez:2012jv} relies on 
