...
The main backgroud to the measurement of DVCS is the coherent $\pi^{0}$ 
production. Therefore, we remove events where a $\pi^{0}$ can be identified 
by invariant mass reconstruction of two photons.  To ensure that the 
interaction occurs at the partonic level and that the DVCS handbag diagram is 
applicable we select events with $Q^{2}>1~[GeV^{2}/c^{2}]$. The exclusivity of 
the events is obtained by applying a set of cuts on the following kinematic 
variables: the co-planarity angle ($\Delta \phi$), the missing energy, the 
missing mass squared, the missing transverse momentum, the missing mass 
...
these cuts are presented in Figure~\ref{fig:kin-cuts}, these are 3$\sigma$ 
cuts except for the missing energy that appears to be too large and for which 
we reduced the cut window to [-0.45,0.5] GeV. After these requirements, we 
have about 3500 events left, Figure \ref{fig:kin-coverage} presents their
...
\phi$, missing energy, missing mass squared and the 
...
%asymmetries
Several observables related to DVCS are of interest, in this work we focus 
only on the beam-spin asymmetry. This  
observable is measurable using a polarized lepton beam on an unpolarized target 
(U). It is convenient to use the beam-spin asymmetry
...
for the nucleon in \cite{Belitsky:2001ns} and for the spinless targets in 
\cite{Kirchner:2003wt,Belitsky:2008bz}. Based on these calculations, the 
beam-spin asymmetry ($A_{LU}$) can then be expressed as:
\small
\begin{equation}
\begin{split}
A_{LU}(\phi) =~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\\
