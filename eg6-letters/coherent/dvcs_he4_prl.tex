\documentclass[nofootinbib,twocolumn,showpacs,prl,superscriptaddress,secnumarabic,amssymb,nobibnotes,aps,floatfix,preprint]{revtex4}
\usepackage{graphicx}
\usepackage{hyperref}
\usepackage{amsmath}
...
Factors (CFF) \cite{Guidal:2013rya}. That is, for a GPD at a given ($x,\xi,t$), 
we can measure experimentally the real ($\mathcal{P} 
\int_{0}^{1}dx[GPD(x,\xi,t)-GPD(-x,\xi,t)] \, C^{+}(x,\xi)$, with $\mathcal{P}$ 
stands for the Cauchy principal value integral and $C^{+}(x,\xi)$ is a 
coefficient function ($=  \frac{1}{x-\xi} + \frac{1}{x+\xi}$)) and the 
imaginary $(GPD(\xi,\xi,t)-GPD(-\xi,\xi,t)$) parts of it's corresponding CFF.

The main challenge in exploring nuclear DVCS is to ensure the exclusivity of 
...
Xe) but has been criticized for mixing coherent and incoherent processes 
\cite{HERMES_BSA}. In this regard, the direct detection of the low-energy 
recoil nuclei is the best way to guarantee that the nucleus is intact and that 
the reaction did not occur on a bound nucleon. The CEBAF Large Acceptance 
Spectrometer (CLAS) setting  in the Hall-B of JLab being already optimized for 
DVCS measurements 
\cite{Mecking:2003zu,Girod:2007aa,Gavalian:2009,Seder:2015,Pisano:2015,Jo:2015ema}, 
we built a dedicated radial time projection chamber (RTPC) for the detection of 
...
$^4$He nuclei produced in DVCS have a low average momentum ($p/q$) around 150 
MeV/c. The CLAS spectrometer cannot detect such low energy $\alpha$ particles, 
...
this RTPC that is surrounding the $^4$He gaseous target and inside the solenoid 
magnet. The detector was specifically calibrated for the detection of
...
model: hep-ph/0509158. (Mohammad to Raphael, the projections shown in figure 5 
are for Carbon-12. Why would we added them?]
