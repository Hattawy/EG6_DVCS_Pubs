...
electron beam from CEBAF machine at Jefferson Lab projected onto a $^4$He gas 
target in the center of the CEBAF Large Acceptance Spectrometer (CLAS). A new 
radial time projection chamber (RTPC) was used to detect the recoiling $^4$He 
nuclei and ensure the exclusivity of the process. The measured Beam Spin 
Asymmetries (BSA) are significantly larger than that observed on the proton in 
the same kinematic domain. Since $^4$He is a spin zero target, we were able to 
extract, in a completely model independent way, the chiral even Generalized 
Parton Distribution (GPD) $H_{A}$ of $^4$He. This pioneering measurement leads 
the way toward the 3D imaging of the partonic structure of nuclei.
\end{abstract}
\pacs{Valid PACS appear here}

\maketitle 

Rich information on Quantum Chromodynamics (QCD) lies in the internal structure 
...
information about the momentum and spatial degrees of freedom of the quark and 
...
has been the focus of a worldwide effort 
...
exclusivity of the reaction, i.e. the target nucleus. Figure \ref{fig:diags} 
...
The main challenge in studying nuclear DVCS is ensuring the exclusivity of the 
reaction. Until now, the only available data on nuclear DVCS was from the 
HERMES experiment \cite{Ellinghaus:2002zw}, where exclusivity was based only on 
...
built a specialized radial time projection chamber (RTPC) for detection of a 
...
using a 6 atm gaseous $^4$He target placed in the center of CLAS. For DVCS 
...
In our kinematics, the recoil 4He nuclei produced in DVCS have a low average 
...
target, a $25$ cm long, $27~\mu$m wall Kapton straw, in the center was part of 
the RTPC assembly. The detector was calibrated specifically for the detection 
...
Electrons are identified using measured momentum, time and energy in fiducial 
volume of CLAS drift chambers, Cerenkov counters, scintillator counters, and 
electromagnetic calorimeters. The $^4$He are identified in the RTPC using time, 
track quality, and energy loss cut for tracks in the fiducial region. In 
...
the Beam Spin Asymmetry (BSA). The BSA on unpolarized target (U), $A_{LU}$, is 
measurable by the difference of cross sections of the reaction at opposite beam 
helicities normalized to the total cross section. It is written as:
  \begin{equation}
...
respect to the beam direction. In this ratio most of uncertainties of the 
...
($e$,$e'$) and ($\gamma^{*}$,$^4$He$'$) planes. The $\alpha_i$’s are kinematic 
factors that depend on azimuthal angle $\phi$, the nuclear form factor 
...
in \cite{Belitsky:2008bz}.  We see that by using the $\sin(\phi)$ and 
...
2 to 4\%. The study of systematics uncertainties showed that the main 
...
imaginary part of the CFF $\mathcal{H}_{A}$ are the free parameters of the fit. 
In Figure \ref{fig:alu90} the $Q^2$, $x_{B}$, and $-t$-dependencies of the 
fitted $A_{LU}$ signals at $\phi$~=~90$^{\circ}$ are shown. The $x_{B}$ and 
...
and non-interacting nucleons, where these nucleons interact independently with 
...
%conclusion
In summary, we presented the first exclusive measurement of coherent DVCS off 
...
low energy $^4$He recoils in order to ensure the DVCS exclusivity.
...
parts of the $^4$He CFF, $\mathcal{H}_A$. The extracted CFF is in a good 
