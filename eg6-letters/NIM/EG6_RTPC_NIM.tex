...
mixture composition, the electric and the magnetic field experienced by the drift 
electrons. This time correspond to the electrons released close to 
the cathode, which take the most time to reach the readout pads as illustrated in 
Figure~\ref{fig:RTPC_signals}. Note that the minimum time is not calibrated, as we
use the trigger time is defined ($T_{min}=15$), and that the time unit is the ALTRO 
sampling time of 100~ns.
...
In order to experimentally determine the value of $T_{max}$, we analyse the
...
measured in bins along the 200~mm RTPC's 
length to take into account variations in the electric and magnetic field in 
the RTPC. We show this result for one experimental run in 
Figure~\ref{fig:RunNumber_61551_TDCmax_Zslice}. 
...
result. Because of the magnetic field, the drift paths in the RTPC are not linear 
but the curvature is small and the process converged already on the 
second iteration.
